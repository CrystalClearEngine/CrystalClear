...
namespace CrystalClear
{
	public static class Utilities
	{
		...
		///     "QWERTY" is renamed to "QWERTY (1)", and if another "QUERTY" is attempted to be added it is changed to "QUERTY (2)"
		///     etc.
		/// </summary>
		...
		public static string EnsureUniqueName(string baseName, IEnumerable<string> otherNames)
		{
			...
			while (otherNames.Contains(baseName))
			{
				...
				{
					baseName = baseName.Remove(baseName.Length - OldDuplicateDecorator.Length,
						OldDuplicateDecorator.Length);
				}

				var DuplicateDecorator = $" ({i})";
		...
		public static bool ReflectionEquals(this object a, object b, bool includePrivate = false,
			bool ignoreProperties = false)
		{
			...
			if (a is null || b is null) // if both aren't, then if one is they can´t be equal
			{
				return false;
			}
			...
			if (includePrivate)
			{
				...
				{
					properties =
						aType.GetProperties(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
				}
			}
			else
			{
